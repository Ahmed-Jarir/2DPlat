...
public class PlayerController : MonoBehaviour
{
    ...
    public int defaultAllowedJumps = 1;
    ...
    private void HandleJumpInput()
    {
        if (jumpInput)
        {
			//GameObject.Find("PlayerSprite").GetComponent<Animator>().Controller = "/Assets/Art/Animation/Player/PlayerAnimatorController";
    ...
    private IEnumerator Jump(float powerMultiplier = 1.0f)
    {
        if (timesJumped < allowedJumps && state != PlayerState.Dead)
        {
            ...
            if (timesJumped > defaultAllowedJumps)
            {
                allowedJumps = defaultAllowedJumps;
            }
               jumping = false;
        }
    }
