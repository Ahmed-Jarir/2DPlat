...
public class UIManager : MonoBehaviour
{
	...
	private void SetUpPlayer(){
        ...
        {
            if (Skins[0] != null) ;
            int index = SelectSkin.getSkin();
            GameObject player = Skins[index];
            player.SetActive(true);
            Cam.GetComponent<CameraController>().SetTarget(player);
            GameObject.Find("GameManager").GetComponent<GameManager>().SetPlayer(player);
        }
